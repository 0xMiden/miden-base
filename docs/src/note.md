...
As with [accounts](./account), notes can be stored either publicly or privately:
...
The [transaction prologue](./transactions) requires all necessary data to compute the `Note` hash. This setup allows scenario-specific restrictions on who may consume a `Note`.
