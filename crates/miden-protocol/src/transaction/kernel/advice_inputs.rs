...
impl TransactionAdviceInputs {
    ...
    fn build_stack(&mut self, tx_inputs: &TransactionInputs) {
        ...
        self.extend_stack([
            ...
            Felt::new(header.version() as u64),
            Felt::new(header.timestamp() as u64),
            ZERO,
        ]);
        self.extend_stack([
            header.fee_parameters().native_asset_id().suffix(),
            header.fee_parameters().native_asset_id().prefix().as_felt(),
            Felt::new(header.fee_parameters().verification_base_fee() as u64),
            ZERO,
        ]);
        ...
        self.extend_stack([Felt::new(tx_inputs.input_notes().num_notes() as u64)]);
    ...
    fn add_account(&mut self, account: &PartialAccount) {
        ...
        self.extend_map(
            account
                .storage()
                .leaves()
                .map(|leaf| (leaf.hash(), leaf.to_elements().collect::<Vec<_>>())),
        );
    ...
    fn add_account_witness(&mut self, witness: &AccountWitness) {
        ...
        self.add_map_entry(leaf.hash(), leaf.to_elements().collect::<Vec<_>>());
    ...
    fn add_input_notes(&mut self, tx_inputs: &TransactionInputs) {
        ...
        for input_note in tx_inputs.input_notes().iter() {
            ...
            note_data.push(Felt::new(recipient.storage().num_items() as u64));
            ...
            match input_note {
                InputNote::Authenticated { note, proof } => {
                    ...
                    note_data.push(Felt::new(proof.location().node_index_in_block() as u64));
                },
                InputNote::Unauthenticated { .. } => {
                    // push the `is_authenticated` flag
                    note_data.push(Felt::new(0))
                },
            }
        }
