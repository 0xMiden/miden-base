...
impl TryFrom<[u8; 15]> for AccountIdV0 {
    ...
    fn try_from(mut bytes: [u8; 15]) -> Result<Self, Self::Error> {
        ...
        let prefix = Felt::new(u64::from_le_bytes(
            prefix_slice.try_into().expect("slice should be 8 bytes"),
        ));

        let suffix = Felt::new(u64::from_le_bytes(suffix_bytes));
...
fn shape_suffix(suffix: Felt) -> Felt {
    let suffix = suffix.as_canonical_u64() & 0xffff_ffff_ffff_ff00;
