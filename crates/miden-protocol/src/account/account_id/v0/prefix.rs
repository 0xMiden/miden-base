...
impl TryFrom<[u8; 8]> for AccountIdPrefixV0 {
    ...
    fn try_from(mut value: [u8; 8]) -> Result<Self, Self::Error> {
        // Felt::new expects a u64 value in native byte order.
        // The value is in big-endian format from the wire format.
        let u64_value = u64::from_be_bytes(value);

        Self::new(Felt::new(u64_value))
    }
}
