...
#[test]
fn decoding_errors_preserve_the_field_path_and_source() {
    ...
    for error in [
        ...
        assert_eq!(error.to_string(), format!("value: {source}"));
        assert!(error.source().unwrap().is::<TryFromIntError>());
    }
}

#[test]
fn construction_errors_preserve_typed_sources_and_existing_context() {
    let source = u8::try_from(256_u32).unwrap_err();
    for (error, expected) in [
        (Message(256).decode_and_verify().unwrap_err(), source.to_string()),
        (
