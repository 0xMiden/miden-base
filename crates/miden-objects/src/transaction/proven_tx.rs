...
pub struct TxAccountUpdate {
    ...
    ///
    /// This must be the commitment to the account delta as computed by the transaction kernel at
    /// the end of transaction execution. Notably, this _excludes_ the automatically removed fee
    /// asset. The account delta possibly contained in [`AccountUpdateDetails`] _includes_
    /// the _removed_ fee asset, so that it represents the full account delta of the transaction.
    /// This mismatch means that in order to validate the delta, the fee asset must be _added_ to
    /// the delta before checking its commitment against this field.
    account_delta_commitment: Word,
