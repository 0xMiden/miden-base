...
use miden_objects::{BuildUnchecked, DecodeMessage, VerifyWith, proto};
...
fn proposal_proofs_are_only_checked_by_explicit_verification() {
    ...
    assert!(
        matches!(
            error,
            miden_objects::decoded::transaction::ProposedBatchError::Batch(
...
fn proven_batch_rejects_changed_proposal_fields() {
    ...
    for field in ["reference block number", "reference block commitment", "expiration block"] {
        ...
        assert!(
            matches!(error, miden_objects::decoded::transaction::ProvenBatchError::ProposalMismatch(actual) if actual == field),
            "{error}"
        );
    }
}
