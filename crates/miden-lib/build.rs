...
fn generate_event_constants(asm_source_dir: &Path, target_dir: &Path) -> Result<()> {
    ...
    let event_file_content = generate_event_file_content(&events)?;
...
fn generate_event_file_content(events: &BTreeMap<String, String>) -> Result<String> {
    use std::fmt::Write;
    ...
    // Generate lookup table for std feature
    {
