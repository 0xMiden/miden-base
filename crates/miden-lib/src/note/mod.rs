...
/// Generates an IF_SWAP note - conditional swap of assets between two accounts - and returns the
/// note as well as [NoteDetails] for the payback note.
///
/// This script enables a conditional swap of 2 assets between the `sender` account and any other
/// account that is willing to consume the note. The consumer will create a new P2ID note with
/// `sender` as target, containing the `requested_asset`, but will NOT receive the `offered_asset`
/// in their account (it stays in the note).
///
/// # Errors
/// Returns an error if deserialization or compilation of the `IF_SWAP` script fails.
pub fn create_if_swap_note<R: FeltRng>(
    sender: AccountId,
    offered_asset: Asset,
    requested_asset: Asset,
    swap_note_type: NoteType,
    ...
    let note_script = WellKnownNote::IFSWAP.script();
    ...
    // build the tag for the IF_SWAP use case
    let tag = build_swap_tag(swap_note_type, &offered_asset, &requested_asset)?;
    let serial_num = rng.draw_word();
