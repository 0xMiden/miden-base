...
mod events;
pub use events::{TransactionEvent, TransactionEventData, TransactionEventHandling};
...
impl TransactionKernel {
    ...
    /// Returns a new Miden assembler instantiated with the transaction kernel and loaded with the
    /// Miden stdlib as well as with miden-lib.
    pub fn assembler_with_source_manager(source_manager: Arc<dyn SourceManager>) -> Assembler {
        #[cfg(all(any(feature = "testing", test), feature = "std"))]
        source_manager_ext::load_masm_source_files(&source_manager);
...
mod source_manager_ext {
    ...
    pub fn load_masm_source_files(source_manager: &Arc<dyn SourceManager>) {
    ...
    fn load(source_manager: &Arc<dyn SourceManager>) -> io::Result<()> {
