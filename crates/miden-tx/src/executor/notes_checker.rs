    ...
    AUTH: TransactionAuthenticator + Sync,
{
    ...
    pub async fn check_notes_consumability(
        ...
        let tx_inputs = self
            .0
            .prepare_transaction_notes(target_account_id, notes.clone().into(), &tx_args, block_ref) // todo clone
            .await
            .expect("todo map error");

        // Attempt to find an executable set of notes.
        self.find_executable_notes_by_elimination(notes, &tx_inputs, &tx_args).await
    }
