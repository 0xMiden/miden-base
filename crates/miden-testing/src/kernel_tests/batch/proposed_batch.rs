...
fn note_created_and_consumed_in_same_batch() -> anyhow::Result<()> {
    ...
    let tx1 =
        MockProvenTxBuilder::with_account(account1.id(), Word::default(), account1.commitment())
            .ref_block_commitment(block1.commitment())
            .output_notes(vec![OutputNote::Full(note.clone())])
            .build()?;
    let tx2 =
        MockProvenTxBuilder::with_account(account2.id(), Word::default(), account2.commitment())
            .ref_block_commitment(block1.commitment())
...
fn multiple_transactions_against_same_account() -> anyhow::Result<()> {
    ...
    let initial_state_commitment = Word::default();
