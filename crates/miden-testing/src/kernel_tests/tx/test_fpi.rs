...
fn test_fpi_memory() -> anyhow::Result<()> {
    ...
    assert_eq!(
        try_read_root_mem_word(process, NATIVE_ACCOUNT_DATA_PTR + ACCOUNT_DATA_LENGTH as u32 * 2),
        None,
...
fn test_fpi_memory_two_accounts() -> anyhow::Result<()> {
    ...
    assert_eq!(
        read_root_mem_word(process, NATIVE_ACCOUNT_DATA_PTR + ACCOUNT_DATA_LENGTH as u32),
        Word::new([
    ...
    assert_eq!(
        read_root_mem_word(process, NATIVE_ACCOUNT_DATA_PTR + ACCOUNT_DATA_LENGTH as u32 * 2),
        Word::new([
    ...
    assert_eq!(
        try_read_root_mem_word(process, NATIVE_ACCOUNT_DATA_PTR + ACCOUNT_DATA_LENGTH as u32 * 3),
        None,
...
fn foreign_account_data_memory_assertions(foreign_account: &Account, process: &Process) {
    ...
    assert_eq!(
        read_root_mem_word(process, foreign_account_data_ptr + ACCT_ID_AND_NONCE_OFFSET),
        Word::new([
    ...
    assert_eq!(
        read_root_mem_word(process, foreign_account_data_ptr + ACCT_VAULT_ROOT_OFFSET),
        foreign_account.vault().root(),
    );

    assert_eq!(
        read_root_mem_word(process, foreign_account_data_ptr + ACCT_STORAGE_COMMITMENT_OFFSET),
        foreign_account.storage().commitment(),
    );

    assert_eq!(
        read_root_mem_word(process, foreign_account_data_ptr + ACCT_CODE_COMMITMENT_OFFSET),
        foreign_account.code().commitment(),
    );

    assert_eq!(
        read_root_mem_word(process, foreign_account_data_ptr + NUM_ACCT_STORAGE_SLOTS_OFFSET),
    ...
    assert_eq!(
        read_root_mem_word(process, foreign_account_data_ptr + NUM_ACCT_PROCEDURES_OFFSET),
