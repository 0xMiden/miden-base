...
fn test_create_note() {
    ...
    let aux = Felt::new(27);
    let tag = NoteTag::from_account_id(account_id).unwrap();
...
fn test_get_output_notes_commitment() {
    ...
    let output_tag_1 = NoteTag::from_account_id(network_account).unwrap();
    ...
    let output_tag_2 = NoteTag::from_account_id(local_account).unwrap();
...
fn test_create_note_and_add_asset() {
    ...
    let recipient = [ZERO, ONE, Felt::new(2), Felt::new(3)];
    let aux = Felt::new(27);
    let tag = NoteTag::from_account_id(faucet_id).unwrap();
...
fn test_create_note_and_add_multiple_assets() {
    ...
    let recipient = [ZERO, ONE, Felt::new(2), Felt::new(3)];
    let aux = Felt::new(27);
    let tag = NoteTag::from_account_id(faucet_2).unwrap();
