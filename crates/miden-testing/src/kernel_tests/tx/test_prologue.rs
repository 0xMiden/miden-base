...
use miden_lib::{
    ...
    transaction::{
        ...
        memory::{
            ...
            INPUT_NOTE_ASSETS_OFFSET, INPUT_NOTE_ID_OFFSET, INPUT_NOTE_METADATA_OFFSET,
            INPUT_NOTE_NULLIFIER_SECTION_PTR, INPUT_NOTE_NUM_ASSETS_OFFSET,
            INPUT_NOTE_PAYLOAD_COMMITMENT_OFFSET, INPUT_NOTE_RECIPIENT_OFFSET,
...
fn input_notes_memory_assertions(
    ...
    for (input_note, note_idx) in inputs.input_notes().iter().zip(0_u32..) {
        ...
        assert_eq!(
            read_note_element(process, note_idx, INPUT_NOTE_PAYLOAD_COMMITMENT_OFFSET),
