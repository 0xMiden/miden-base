...
use miden_lib::{
    ...
    transaction::{
        ...
        memory::{
            ...
            INPUT_NOTE_RECIPIENT_COMMITMENT_OFFSET, INPUT_NOTE_SCRIPT_ROOT_OFFSET,
            INPUT_NOTE_SECTION_PTR, INPUT_NOTE_SERIAL_NUM_OFFSET, INPUT_NOTES_COMMITMENT_PTR,
            KERNEL_PROCEDURES_PTR, MemoryOffset, NATIVE_ACCT_CODE_COMMITMENT_PTR,
...
fn input_notes_memory_assertions(
    ...
    for (input_note, note_idx) in inputs.input_notes().iter().zip(0_u32..) {
        ...
        assert_eq!(
            read_note_element(process, note_idx, INPUT_NOTE_RECIPIENT_COMMITMENT_OFFSET),
            note.recipient().digest(),
            "note recipient commitment should be stored at the correct offset"
        );
