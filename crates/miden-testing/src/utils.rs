...
macro_rules! assert_execution_error {
    ($execution_result:expr, $expected_err:expr) => {
        match $execution_result {
            Err($crate::ExecError(miden_processor::ExecutionError::OperationError { err: miden_processor::OperationError::FailedAssertion { err_code, err_msg }, .. })) => {
                let err_code: miden_processor::Felt = err_code;
    ...
    ($execution_result:expr, $expected_err:expr) => {
        match $execution_result {
            Err(::miden_tx::TransactionExecutorError::TransactionProgramExecutionFailed(
                ::miden_processor::ExecutionError::OperationError { err: ::miden_processor::OperationError::FailedAssertion { err_code, .. }, .. },
            )) => {
                assert_eq!(
                  err_code, $expected_err.code(),
                  "Execution failed on assertion with an unexpected error code (Actual: {}, Expected: {}).",
