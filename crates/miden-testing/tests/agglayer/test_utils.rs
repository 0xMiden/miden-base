...
pub async fn execute_program_with_default_host(
    ...
    for (event_name, handler) in std_lib.handlers() {
        host.register_handler(event_name, handler).map_err(ExecError::new)?;
    }
    ...
    processor.execute(&program, &mut host).await.map_err(ExecError::new)
}
