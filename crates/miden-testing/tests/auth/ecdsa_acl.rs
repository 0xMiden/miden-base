...
async fn test_ecdsa_acl() -> anyhow::Result<()> {
    ...
    let tx_script_trigger_1 =
        CodeBuilder::with_mock_libraries().parse_tx_script(tx_script_with_trigger_1)?;

    let tx_script_trigger_2 =
        CodeBuilder::with_mock_libraries().parse_tx_script(tx_script_with_trigger_2)?;

    let tx_script_no_trigger =
        CodeBuilder::with_mock_libraries().parse_tx_script(TX_SCRIPT_NO_TRIGGER)?;
