...
- Added the `pass_through::single_p2id` transaction script with its `PassThroughSingleP2idTransactionScript` type and the `PassThrough` account component, forwarding the account's balance of the listed assets into a single P2ID note; keeping the account unchanged additionally requires the `AuthPassThrough` component, which is not yet available ([#3709](https://github.com/0xMiden/protocol/issues/3709)).
