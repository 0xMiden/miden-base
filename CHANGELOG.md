...
- Added the `AuthPassThrough` auth component, which forwards the single asset of every consumed note into one P2ID note addressed by the auth args, verifies a signature over the transaction summary and leaves the account unchanged ([#3844](https://github.com/0xMiden/protocol/pull/3844)).
