...
- Added the `pass_through::single_p2id` transaction script with its `PassThroughSingleP2idTransactionScript` type, the `PassThroughSweep` account component forwarding the account's balance of the listed assets into a single P2ID note, and the `AuthPassThrough` account component that authenticates such a transaction by signature while leaving the account's state and nonce untouched after the transaction that creates it, and creating no `TX_FEE` note ([#3709](https://github.com/0xMiden/protocol/issues/3709)).
