...
- [BREAKING] `NetworkAccountTarget` decoding no longer discards the target account ID when the execution hint slot holds an unrecognized encoding, which now decodes as the new `NoteExecutionHint::Unknown` variant; `NoteExecutionHint`'s `u64` conversions are replaced by lossless `Felt` ones ([#3811](https://github.com/0xMiden/protocol/pull/3811)).
